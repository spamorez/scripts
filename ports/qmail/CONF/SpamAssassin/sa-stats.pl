...
$opt{'from'} = 'Spamorez System Admin';    # Who is the mail from
$opt{'end'} = "";
$opt{'start'} = "today";
...
if ($opt{'mail'}) {
    ...
    print SENDMAIL "Subject: Spamorez statistics\n\n";
    print SENDMAIL $rpt;
    close (SENDMAIL);
} else {
    print $rpt;
...
sub build_text_report {
    ...
    my $rpt = '';
    $rpt .=         "Report Title     : Spamorez - Spam Statistics\n";
...
<html>
<head>
<title>Spamorez Statistics: $d_start - $d_end</title>
</head>
<body>
<h1>Spamorez Statistics:</h1>
<p>
